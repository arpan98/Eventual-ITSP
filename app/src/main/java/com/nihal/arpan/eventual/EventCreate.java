...
public class EventCreate extends AppCompatActivity {
    String TAG = "swag",existingOId="",QRStart="EVENTualQR",seq="~!#";
    ...
    public void save(View v) {
        ...
        /*
            Just give up if you have to debug again. You will definitely not understand all the if's and else's
         */
        try {
            if (isNetworkConnected()) {
                ...
                if (!startdatedisplay.getText().toString().equals("") && !enddatedisplay.getText().toString().equals("")) {
                    ...
                    final ParseObject eventData = new ParseObject("EventData");
                    if (allday) {
                        ...
                        eventData.put("starthour", "");
                        eventData.put("startminute", "");
                        eventData.put("endhour", "");
                        eventData.put("endminute", "");
                    } else {
                        startdate = (startdatedisplay.getText().toString()).substring(0, 2);
                        startmonth = (startdatedisplay.getText().toString()).substring(3, 5);
                        startyear = (startdatedisplay.getText().toString()).substring(6, 10);
                        ...
                        endmonth = (enddatedisplay.getText().toString()).substring(3, 5);
                        endyear = (enddatedisplay.getText().toString()).substring(6, 10);
                        if (!starttimedisplay.getText().toString().equals("") && !endtimedisplay.getText().toString().equals("")) {
                    ...
                    if (!(endtimestamp >= starttimestamp)) {
                        ...
                        eventData.put("allday", allday);
                        eventData.put("startyear", startyear);
                        eventData.put("startmonth", startmonth);
                        eventData.put("startdate", startdate);
                        eventData.put("endyear", endyear);
                        eventData.put("endmonth", endmonth);
                        eventData.put("enddate", enddate);
                        eventData.put("location", location);
                        if (!allday) {
                            eventData.put("starthour", starthour);
                            eventData.put("startminute", startminute);
                            eventData.put("endhour", endhour);
                            eventData.put("endminute", endminute);
                        }
                        ...
                        query.getFirstInBackground(new GetCallback<ParseObject>() {
                            public void done(ParseObject object, ParseException e) {
                                if (object == null) {
                                    eventData.saveInBackground(new SaveCallback() {
                                        @Override
                                        public void done(ParseException e) {
                                            ...
                                            if (e == null) {
                                                ...
                                                final String objectId = eventData.getObjectId();
                                                ...
                                                Toast.makeText(EventCreate.this,"Error in saving event",Toast.LENGTH_LONG).show();
                                    ...
                                    dialog.dismiss();
                                    savebutton.setEnabled(true);
                                    existingOId = object.getObjectId();
                                    WindowManager manager = (WindowManager) getSystemService(WINDOW_SERVICE);
                                    Display display = manager.getDefaultDisplay();
                                    Point point = new Point();
                                    display.getSize(point);
            ...
            Log.d(TAG,"Save event function - tu to gaya beta = "+e.toString());
            Toast.makeText(getApplicationContext(), "An error occured! Please post a bug report on the playstore page", Toast.LENGTH_LONG).show();
        }
    }
